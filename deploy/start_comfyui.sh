...
if curl -fsS --max-time 2 -o /dev/null "http://127.0.0.1:${H3_COMFYUI_PORT}/"; then
...
for _ in $(seq 1 "${H3_READY_TIMEOUT_SEC:-30}"); do
...
echo "[h3] ComfyUI 尚未响应，日志: $H3_LOG_DIR/comfyui.log" >&2
exit 1
